#!/usr/bin/env python3
"""
Upload GCP Checks to OpenSearch Index

This script reads the GCP checks CSV with LLM risk labels and uploads
...
class GCPChecksUploader:
    """Handles uploading GCP checks to OpenSearch."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Upload GCP checks to OpenSearch')
    parser.add_argument('--csv', '-c',
                       default=str(PROJECT_ROOT / 'secure' / 'gcp' / 'opensearch_work' / 'data' / 'gcp' / 'gcp_checks_with_llm_risk_labels.csv'),
                       help='Path to CSV file (default: secure/gcp/opensearch_work/data/gcp/gcp_checks_with_llm_risk_labels.csv)')
    ...
    try:
        uploader = GCPChecksUploader(index_name=args.index)
        ...
        logger.info("=" * 80)
        logger.info("Uploading GCP Checks to OpenSearch")
        logger.info("=" * 80)
