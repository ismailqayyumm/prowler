#!/usr/bin/env python3
"""
Merge LLM risk labeling results with Azure checks CSV.

This script:
1. Loads the LLM results from results/all_azure_results.json
2. Loads the original Azure CSV
...
6. Outputs to: opensearch_work/data/azure_checks_with_llm_risk_labels.csv
"""
...
def main():
    """Main merge function."""
    print("=" * 80)
    print("Merging LLM Results with Azure Checks CSV")
    print("=" * 80)
    ...
    # Load LLM results
    results_file = Path(__file__).parent / "results" / "all_azure_results.json"
    if not results_file.exists():
        print(f"\n❌ Error: LLM results file not found: {results_file}")
        print("   Run azure_mapper.py first to generate LLM results.")
        sys.exit(1)
    ...
    # Find Azure CSV
    csv_dir = Path(__file__).parent / "../opensearch_work/data/azure"
    azure_files = list(csv_dir.glob("azure_checks_extracted_*.csv"))

    if not azure_files:
        print(f"\n❌ Error: No Azure CSV found in {csv_dir}")
        sys.exit(1)

    csv_file = max(azure_files, key=lambda p: p.stat().st_mtime)
    print(f"✓ Using Azure CSV: {csv_file.name}")
    ...
    output_file = csv_dir / "azure_checks_with_llm_risk_labels.csv"
...
if __name__ == "__main__":
    try:
        ...
        print("\nNext step: Run upload_azure_checks.py to upload to OpenSearch index")
    except Exception as e:
